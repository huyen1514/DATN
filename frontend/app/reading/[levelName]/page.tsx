...
import { FileText, CopyPlus, ArrowRight } from "lucide-react";
    ...
    <StudentLayout>
      <div className="max-w-5xl mx-auto">
        <div className="mb-10">
          ...
          <p className="text-neutral-500 font-light">Chọn bài học để luyện tập kỹ năng Đọc hiểu trình độ {levelName.toUpperCase()}</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white p-16 rounded-3xl border border-black/5 text-center">
            ...
            <p className="text-neutral-500">Nội dung trình độ {levelName} đang được cập nhật thêm bài đọc.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {lessons.map(lesson => (
              <Link 
                ...
                className="group bg-white rounded-3xl border border-black/5 p-8 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-indigo-500/20 transition-all duration-300 block relative overflow-hidden"
              >
                ...
                <div className="relative z-10">
                  <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full mb-4 inline-block tracking-widest uppercase">
                    Bài học
                  </span>
                  <h3 className="text-xl font-bold text-jp-indigo mb-2 group-hover:text-indigo-600 transition-colors">{lesson.LessonName || `Bài ${lesson.lessonId}`}</h3>
                  
                  <div className="mt-8 flex items-center justify-between text-sm">
                    <span className="font-bold text-neutral-400">{readingCounts[lesson.lessonId] || 0} Đoạn văn</span>
